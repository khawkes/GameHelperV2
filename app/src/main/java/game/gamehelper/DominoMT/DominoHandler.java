...
import game.gamehelper.ObjectHandler;

/**
 * Class responsible for categorizing and handling shapes in the image data.
 *
 * Created by Mark Andrews on 4/11/2015.
 */
public class DominoHandler implements ObjectHandler
{
    ...
    public DominoHandler(int width, int height)
    {
    ...
    {
        for(ArrayList<Point> p: cornerList)
        {
            //checking if length is about twice as long as height
            boolean check1 = checkLong(p.get(0), p.get(1), p.get(2));

            //checking if length is about the same as height
            boolean check2 = checkSquare(p.get(0), p.get(1), p.get(2), p.get(3));

            //add object to corresponding list
            if (check1)
            {
                addShape(p.get(0), p.get(1), p.get(2), p.get(3), 0);
            }
            else if (check2)
            {
                addShape(p.get(0), p.get(1), p.get(2), p.get(3), 1);
            }
        }
    ...
    private boolean addShape(Point lb, Point lt, Point rt, Point rb, int option)
    {
    ...
    private int countSide(int side, Point[] a)
    {
    ...
    private void deleteOutliers(int rem)
    {
        if( rem <= 0 )
        {
    ...
    private boolean isInside(Point e, Point a, Point b, Point c, Point d)
    {
    ...
    private void drawShape(Point[] points, int option)
    {
        if(option == 0)
        {
